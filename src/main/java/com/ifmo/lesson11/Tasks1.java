...
public class Tasks1 {
    ...
    private static void countEachPriority(List<Message> messages) {
        ...
        Map<MessagePriority, Integer> countPriority = new HashMap<>();
        for (Message m : messages){
            countPriority.merge(m.getPriority(), 1, (oldVal, newVal) -> oldVal + newVal);
        }
    ...
    private static void countCountEachCode(List<Message> messages) {
        ...
        // Ответ необходимо вывести в консоль.
        Map<Integer, Integer> countCode = new HashMap<>();
        for (Message m : messages){
            countCode.merge(m.getCode(), 1, (oldVal, newVal) -> oldVal + newVal);
        }
    ...
    private static void countUniqueMessages(List<Message> messages) {
        ...
        // Ответ необходимо вывести в консоль.
        System.out.println(new HashSet<>(messages).size());
    }
